        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_position[0]][next_position[1]]:
                if next_position in self.corners and next_position not in visited_corners:
                    new_visited_corners = tuple(list(visited_corners) + [next_position])
                    successors.append(((next_position, new_visited_corners), action, 1))
                else:
                    successors.append(((next_position, visited_corners), action, 1))
...
#         corners = problem.corners
#
#         if set(visited_corners) == set(corners):
#             return 0
#
#         unvisited_corners = set(corners) - set(visited_corners)
#
#         if unvisited_corners:
#             total_distance = sum(util.manhattanDistance(current_position, corner) for corner in unvisited_corners)
#             average_distance = total_distance / len(unvisited_corners)
#
#             max_distance = max(util.manhattanDistance(current_position, corner) for corner in unvisited_corners)
#
#             return max(average_distance, max_distance)
#         else:
#             return 0
def cornersHeuristic(state: Any, problem: CornersProblem):
    current_position, visited_corners = state
